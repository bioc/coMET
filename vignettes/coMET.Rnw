...
To install \Biocpkg{coMET} from the development version of Bioconductor, the user must install R-devel from \url{http://www.bioconductor.org/developers/how-to/useDevel/}. Following this installation, use standard Bioconductor command line, e.g.
...
\item \Robject{tracks.gviz}, \Robject{tracks.ggbio}, \Robject{tracks.trackviewer} (for \Rfunction{comet} function): For each option, it is possible to give a list of annotation tracks that is created by the \Biocpkg{Gviz}, \Biocpkg{GGBio}, and \Biocpkg{TrackViewer} bioconductor packages. The integration of plots from ggbio and trackviewer can be sometimes not really perfect. It is better to create plots from \Biocpkg{Gviz} and use \Robject{tracks.gviz}
\end{itemize}
...
\item \Rfunction{otherRegulatoryRegions\_ENSEBML} : Visualise the other regulatory regions in the genomic region of interest
...
\item \Rfunction{regulatorySegmentsBiomart\_ENSEBML} : Visualise the regulatory segment regions in the genomic region of interest
...
\begin{itemize}
    ...
    \item \Rfunction{COSMIC\_UCSC} : Visualise SNPs found in COSMIIC tracks of UCSC genome browser in the genomic region of interest.
    ...
    \item \Rfunction{DNAse\_UCSC} : Visualise clinical CNV found in ClinVar tracks of UCSC genome browser in the genomic region of interest.
...
For 18 and 25 states, there are the choice beteen 2 set of colors. First, the colors defined by NIH Roadmap and second, the colors defined by us for a better differentiation between states.
